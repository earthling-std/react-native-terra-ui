...
Add the [Unistyles Babel plugin](https://www.unistyl.es/v3/start/getting-started/#installation) to your `babel.config.js`, pointing `root` at your app's source folder. Include `react-native-terra-ui` in `autoProcessImports` so the library's own `StyleSheet.create` calls are processed too, and keep the Reanimated `worklets` plugin **last**:
    ...
    plugins: [
      ...
      [
        'react-native-unistyles/plugin',
        {
          ...
          // also process react-native-terra-ui's own styles
